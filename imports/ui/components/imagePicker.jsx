...
export default function ImagePicker(props) {
  ...
  const { collectionId , imageType} = props;
...
//each time cropData is updated, we upload the image
  ...
  const {imageLink, isLoading }  = useTracker(() =>  {
    let isLoading = true;
    if(!collectionId) {return { imageLink : null, isLoading:true }}
    const imageFile = Images.findOne({'meta.objectId' : collectionId, 'meta.imageType': imageType});
    if(imageFile) {
      const imageLink = imageFile.link()
      return { imageLink, isLoading: false }; 
    } else {
      return { imageLink : null, isLoading:true }
    }
  ...
  const deleteImage = (id, type) => {
    Meteor.call('image.remove', id, type);
  }
    ...
    if (cropData !== "#") {
      ...
      // there was multiple files selected
        ...
        let uploadInstance = Images.insert({
          ...
          fileName: collectionId + '.png', // <— Mandatory
          type: 'image/png',
          meta: {
    ...
    <div>
      <Box
        ...
        {!inProgress && <Avatar
              ...
              src={ imageLink ? imageLink: cropData ? cropData : "/images/avatar_male.png"}
            />}
        ...
        { (collectionId && imageLink) && <Fab color="primary" size="small" component="span" aria-label="edit" className={classes.editIcon}>
          <DeleteIcon onClick={() => deleteAvatar(collectionId)}/>
          </Fab>}
